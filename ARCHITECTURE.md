...
internal/
  api/              JSON HTTP API (/api/v1/*). Spec-first — openapi.yaml is
                    the source of truth; server.gen.go is generated by
                    oapi-codegen and committed.
...
Spec-first OpenAPI 3.0.3 at `internal/api/openapi.yaml`. `oapi-codegen`
generates `server.gen.go` (server interface + types) into the same
